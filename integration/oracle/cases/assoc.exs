...
defmodule Ecto.Integration.AssocTest do
  ...
  test "has_one assoc" do
    ...
    %Permalink{id: lid1} = TestRepo.insert!(%Permalink{id: 1, url: "1", post_id: p1.id})
    %Permalink{}         = TestRepo.insert!(%Permalink{id: 2, url: "2"})
    %Permalink{id: lid3} = TestRepo.insert!(%Permalink{id: 3, url: "3", post_id: p2.id})
  ...
  test "belongs_to assoc" do
    ...
    l1 = TestRepo.insert!(%Permalink{id: 1, url: "1", post_id: pid1})
    l2 = TestRepo.insert!(%Permalink{id: 2, url: "2"})
    l3 = TestRepo.insert!(%Permalink{id: 3, url: "3", post_id: pid2})
  ...
  test "has_many through-through assoc leading" do
    ...
    u1 = TestRepo.insert!(%User{id: 1})
    u2 = TestRepo.insert!(%User{id: 2})

    pl1 = TestRepo.insert!(%Permalink{id: 1, user_id: u1.id, url: "zzz"})
    pl2 = TestRepo.insert!(%Permalink{id: 2, user_id: u2.id, url: "aaa"})
  ...
  test "has_many through-through assoc trailing" do
    p1  = TestRepo.insert!(%Post{id: 1})
    u1  = TestRepo.insert!(%User{id: 1})
    pl1 = TestRepo.insert!(%Permalink{id: 1, user_id: u1.id, post_id: p1.id})
  ...
  #   p3 = TestRepo.insert!(%Post{id: 3, title: "3", text: "hello"})
  #
  #   %User{id: uid1} = TestRepo.insert!(%User{id: 1, name: "john"})
  #   %User{id: uid2} = TestRepo.insert!(%User{id: 2, name: "mary"})
  #
  ...
  test "has_one changeset assoc (on_replace: :delete)" do
    ...
    changeset =
      ...
      |> Ecto.Changeset.put_assoc(:permalink, %Permalink{id: 1, url: "1"})
    ...
    changeset =
      post
      |> Ecto.Changeset.change
      |> Ecto.Changeset.put_assoc(:permalink, %Permalink{id: 2, url: "2"})
    ...
    existing = TestRepo.insert!(%Permalink{id: 3, url: "3"})
    changeset =
      post
      |> Ecto.Changeset.change
  ...
  #   assoc_custom = TestRepo.insert!(%Custom{id: 1})
  #   custom = TestRepo.insert!(%Custom{id: 1, customs: [assoc_custom]})
  #
  ...
  test "has_many changeset assoc with constraints" do
    author = TestRepo.insert!(%User{id: 1, name: "john doe"})
    p1 = TestRepo.insert!(%Post{id: 1, title: "hello", author_id: author.id})
    TestRepo.insert!(%Post{id: 2, title: "world", author_id: author.id})

    # Asserts that `unique_constraint` for `uuid` exists
    assert_raise Ecto.ConstraintError, fn ->
      TestRepo.insert!(%Post{id: 3, title: "another", author_id: author.id, uuid: p1.uuid})
    end
  ...
  test "belongs_to changeset assoc" do
    # Insert new
    changeset =
      %Permalink{id: 1, url: "1"}
      |> Ecto.Changeset.change
      |> Ecto.Changeset.put_assoc(:post, %Post{id: 1, title: "1"})
    ...
    changeset =
      perma
      |> Ecto.Changeset.change
      |> Ecto.Changeset.put_assoc(:post, %Post{id: 2, title: "2"})
    ...
    existing = TestRepo.insert!(%Post{id: 3, title: "3"})
    changeset =
      perma
      |> Ecto.Changeset.change
  ...
  test "inserting struct with associations" do
    tree = %Permalink{
      ...
      post: %Post{
        ...
        title: "belongs_to",
        comments: [
          %Comment{id: 1, text: "child 1"},
          %Comment{id: 2, text: "child 2"},
        ]
      }
    }
  ...
  test "inserting struct with empty associations" do
    permalink = TestRepo.insert!(%Permalink{id: 1, url: "root", post: nil})
    assert permalink.post == nil

    post = TestRepo.insert!(%Post{id: 1, title: "empty", comments: []})
    assert post.comments == []
  end
  ...
  test "has_many assoc on delete deletes all" do
    ...
    TestRepo.insert!(%Comment{id: 1, post_id: post.id})
    TestRepo.insert!(%Comment{id: 2, post_id: post.id})
  ...
  test "has_many assoc on delete nilifies all" do
    ...
    TestRepo.insert!(%Comment{id: 1, author_id: user.id})
    TestRepo.insert!(%Comment{id: 2, author_id: user.id})
  ...
  #   p1 = TestRepo.insert!(%Post{id: 1, title: "1", text: "hi"})
  #   p2 = TestRepo.insert!(%Post{id: 2, title: "2", text: "hello"})
  #
