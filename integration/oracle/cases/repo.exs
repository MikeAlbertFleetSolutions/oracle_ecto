...
defmodule Ecto.Integration.RepoTest do
  ...
  test "fetch with in" do
    TestRepo.insert!(%Post{id: 1, title: "hello"})
  ...
  test "fetch without schema" do
    %Post{} = TestRepo.insert!(%Post{id: 1, title: "title1"})
    %Post{} = TestRepo.insert!(%Post{id: 2, title: "title2"})
  ...
  test "insert, update and delete" do
    post = %Post{id: 1, title: "insert, update, delete", text: "fetch empty"}
    meta = post.__meta__
  ...
  #   post = TestRepo.insert!(%Post{id: 1, title: "post title", text: "post text"})
  #
  #   user_post = TestRepo.insert!(%PostUserCompositePk{id: 1, user_id: user.id, post_id: post.id})
  #   assert TestRepo.get_by!(PostUserCompositePk, [id: 1, user_id: user.id, post_id: post.id]) == user_post
  ...
  test "insert, update and delete with invalid prefix" do
    post = TestRepo.insert!(%Post{id: 1})
    changeset = Ecto.Changeset.change(post, title: "foo")
    assert catch_error(TestRepo.insert(%Post{id: 2}, prefix: "oops"))
  ...
  test "insert autogenerates for custom type" do
    post = TestRepo.insert!(%Post{id: 1, uuid: nil})
  ...
  test "insert and fetch a schema with timestamps with usec" do
    p1 = TestRepo.insert!(%PostUsecTimestamps{id: 1, title: "hello"})
  ...
  test "unique pseudo-constraint violation error message with join table at the repository" do
    post =
      TestRepo.insert!(%Post{id: 1, title: "some post"})
      |> TestRepo.preload(:unique_users)

    user =
      TestRepo.insert!(%User{id: 1, name: "some user"})
  ...
  test "unique constraint violation error message with join table in single changeset" do
    ...
    {:error, changeset} =
      ...
      |> Ecto.Changeset.unique_constraint(:user,
          name: :puc_pk_post_id_user_id_index,
  ...
  test "unique constraint violation error message with join table and separate changesets" do
    ...
    user = TestRepo.insert!(%User{id: 1, name: "some user"})
  ...
  test "foreign key constraint" do
    changeset = Ecto.Changeset.change(%Comment{id: 1, post_id: 0})
  ...
  test "no assoc constraint error" do
    user = TestRepo.insert!(%User{id: 1})
    TestRepo.insert!(%Permalink{id: 1, user_id: user.id})
  ...
  test "get_by(!)" do
    post1 = TestRepo.insert!(%Post{id: 1, title: "1", text: "hai"})
    post2 = TestRepo.insert!(%Post{id: 2, title: "2", text: "hello"})
  ...
  test "aggregate" do
    ...
    TestRepo.insert!(%Post{id: 3, visits: 14})
    TestRepo.insert!(%Post{id: 4, visits: 14})
  ...
  test "update all" do
    assert %Post{id: id1} = TestRepo.insert!(%Post{id: 1, title: "1"})
    assert %Post{id: id2} = TestRepo.insert!(%Post{id: 2, title: "2"})
    assert %Post{id: id3} = TestRepo.insert!(%Post{id: 3, title: "3"})
  ...
  test "update all increment syntax" do
    assert %Post{id: id1} = TestRepo.insert!(%Post{id: 1, title: "1", visits: 0})
    assert %Post{id: id2} = TestRepo.insert!(%Post{id: 2, title: "2", visits: 1})
  ...
  test "update all with casting and dumping on id type field" do
    assert %Post{id: id1} = TestRepo.insert!(%Post{id: 1})
  ...
  test "delete all" do
    assert %Post{} = TestRepo.insert!(%Post{id: 1, title: "1", text: "hai"})
    assert %Post{} = TestRepo.insert!(%Post{id: 2, title: "2", text: "hai"})
    assert %Post{} = TestRepo.insert!(%Post{id: 3, title: "3", text: "hai"})
  ...
  test "delete all with returning with schema" do
    assert %Post{id: id1} = TestRepo.insert!(%Post{id: 1, title: "1", text: "hai"})
    assert %Post{id: id2} = TestRepo.insert!(%Post{id: 2, title: "2", text: "hai"})
    assert %Post{id: id3} = TestRepo.insert!(%Post{id: 3, title: "3", text: "hai"})
  ...
  test "virtual field" do
    assert %Post{id: id} = TestRepo.insert!(%Post{id: 1, title: "1", text: "hai"})
  ...
  test "query select expressions" do
    %Post{} = TestRepo.insert!(%Post{id: 1, title: "1", text: "hai"})
  ...
  test "query select take with structs" do
    %{id: pid1} = TestRepo.insert!(%Post{id: 1, title: "1"})
    %{id: pid2} = TestRepo.insert!(%Post{id: 2, title: "2"})
    %{id: pid3} = TestRepo.insert!(%Post{id: 3, title: "3"})
  ...
  test "query select take with assocs" do
    %{id: pid} = TestRepo.insert!(%Post{id: 1, title: "post"})
    TestRepo.insert!(%Comment{id: 1, post_id: pid, text: "comment"})
  ...
  test "query select take with single nil column" do
    %Post{} = TestRepo.insert!(%Post{id: 1, title: "1", counter: nil})
    assert %{counter: nil} =
           TestRepo.one(from p in Post, where: p.title == "1", select: [:counter])
  end

  test "query select take with nil assoc" do
    %{id: cid} = TestRepo.insert!(%Comment{id: 1, text: "comment"})
  ...
  test "query count distinct" do
    ...
    TestRepo.insert!(%Post{id: 2, title: "1"})
    TestRepo.insert!(%Post{id: 3, title: "2"})
  ...
  test "query where interpolation" do
    post1 = TestRepo.insert!(%Post{id: 1, text: "x", title: "hello"})
    post2 = TestRepo.insert!(%Post{id: 2, text: "y", title: "goodbye"})
    ...
    post3 = TestRepo.insert!(%Post{id: 3, text: "y", title: "goodbye", uuid: nil})
  ...
  test "load" do
    ...
    columns = Enum.map(result.columns, &String.downcase(&1))
    posts = Enum.map(result.rows, &TestRepo.load(Post, {columns, &1}))
    assert [%Post{title: "title1", inserted_at: ^inserted_at, public: 0}] = posts
  end
  ...
  describe "upsert via insert" do
    ...
    test "on conflict raise" do
      {:ok, inserted} = TestRepo.insert(%Post{id: 1, title: "first"}, on_conflict: :raise)
    ...
    test "on conflict with inc" do
      ...
      post = %Post{id: 1, title: "first", uuid: uuid}
  ...
  describe "upsert via insert_all" do
    ...
    @tag :without_conflict_target
    test "on conflict replace_all" do
      post_first = %Post{title: "first", public: 1}
      post_second = %Post{title: "second", public: 0}
